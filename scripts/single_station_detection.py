...
model_pnw = sbm.EQTransformer.from_pretrained(config['models']['pretrained'])
model_pnw.to(torch.device("cuda"))
model_pnw.default_args = config['models']['default_args']
print(f"{rank} | \tloaded model ({config['models']['pretrained'].upper()}) to GPU:{gpuid}", flush=True)
...
for idx, i in jobs.iterrows():
    ...
    if len(s) > 0:
        if len(s.get_gaps()) > config['models']['max_gap']:
            print(f"{rank} | \t{year}.{sdoy}.{network}.{station} \t| Skip: too many gaps", flush=True)
        else:
            if verbose>1:
                print(f"{rank} | \t{year}.{sdoy}.{network}.{station} \t|", s, flush=True)
            elif verbose>0:
                current_time = time.strftime("%Z %x %X")
                print(f"{rank} | \t{year}.{sdoy}.{network}.{station} \t| {current_time}", flush=True)
            try:
                picks = []
                s.resample(100)
                s.merge()
                ...
                picks, _ = model_pnw.classify(s, strict = False)
                print(f"{rank} | \t{year}.{sdoy}.{network}.{station} \t| Finish, found {len(picks)} picks \t | {'%.3f' % (time.time() - t0)} sec", flush=True)
                
                if len(picks) > 0:
                    # dumping day detection
                    os.makedirs(f"{picks_path}/{network}/{year}/{sdoy}", exist_ok = True)
                    with open(f"{picks_path}/{network}/{year}/{sdoy}/{station}.{network}.{year}.{sdoy}", 'wb') as f:
                        pickle.dump(picks, f)
                    if verbose>1:
                        print(f"{rank} | \tdump catalog to {picks_path}/{network}/{year}/{sdoy}/{station}.{network}.{year}.{sdoy}")
                del picks, s
                gc.collect()

            except:
                print(f"{rank} | \t{year}.{sdoy}.{network}.{station} \t| Error", flush=True)
    else:
        if verbose>0:
            print(f"{rank} | \t{year}.{sdoy}.{network}.{station} \t| Skip: no data", flush=True)
